...
def plot_grid(struct, outdir, metric, metric_col, y_label, smoothing=None):
    ...
    for i, model in enumerate(models):
        for j, lang in enumerate(langs):
            ...
            for p in sorted(cell.keys()):
                ...
                ax.plot(x, y, label=p, color=color_map.get(p), marker='o', markersize=3, linewidth=1)
                any_plotted = True
    ...
    # collect legend entries from axes
    handles = []
    labels = []
    for p in pefts:
    ...
    outpath_png = os.path.join(outdir, f"{model}_{metric}_{metric_col}.png")
